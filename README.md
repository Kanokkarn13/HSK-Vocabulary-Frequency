...
| Security | Bandit (SAST), SonarCloud (quality + hotspots), Dependabot (dependency CVEs + auto-update PRs), OWASP ZAP (DAST — planned) |
| Cloud | Vercel (hosting), Neon (serverless Postgres) — all free tier |
| Monitoring | UptimeRobot |
| Analytics | PostHog (planned) |
| SEO | Google Search Console, Google Lighthouse, Ahrefs Webmaster Tools (planned) |
...
**Current status:** live at
[hsk-vocabulary-frequency.vercel.app](https://hsk-vocabulary-frequency.vercel.app)
(a Vercel-provided subdomain, not a custom domain — see
[Analytics & SEO](wiki/analytics-seo.md) for why that matters for the
Cloudflare plan). `master` has merged both
